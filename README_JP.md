...
#### 📰 LLM-NAKOJA — LLMベース 韓日関係 中立記事生成器
> `Python` `Streamlit` `OpenAI` `Selenium` `BeautifulSoup` `Pandas`

中央日報と読売新聞から韓日関係のニュースをスクレイピングし、GPTでトピックごとにクラスタリングした後、両者の視点を反映した中立的な記事を自動生成するStreamlitパイプラインです。

- **自動化パイプライン**: Selenium/BS4によるスクレイピングからOpenAI SDK v1.xを活用したLLM処理まで、エンドツーエンドで統合しました。
- **トピックのクラスタリングとペアリング**: GPTを活用して収集した記事を広範なトピックに分類し、同一の特定イシューを扱う最適な韓日記事のペアをマッチングします。
- **視点中立化プロンプティング**: 両メディアの報道視点の違いを明示的に浮き彫りにし、韓国語と日本語の2つのバージョンでバランスの取れた中立記事を合成するようLLMを制御します。
